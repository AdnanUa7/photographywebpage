...
if (isset($_GET['value'])) {
	$id = $_GET['value'];
	$email = $_GET['email'];
	mysqli_query($sql_con,"UPDATE booking SET status='2' where id = '$id'");
	// echo "<script>alert('Booking declined successfully !!!')</script>";
...
$mail = new PHPMailer();
  ...
  $mail->Body = "<i style='color:red'>Your Bookings has declined by admin please try again !!!</i>";
  $mail->AltBody = "";
  if(!$mail->send())
  {
